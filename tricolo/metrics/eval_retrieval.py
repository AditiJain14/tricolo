...
def compute_metrics(dataset, embeddings_dict, embeddings_path, metric='cosine', concise=False):
    ...
    if concise is False or isinstance(concise, str):
        if dataset=="shapenet":
            json_path = "../data/text2shape-data/shapenet/shapenet.json"
        if dataset=="primitives":
            json_path = "../data/text2shape-data/primitives/primitives.json"
...
def main():
    ...
    parser.add_argument('--embeddings_path', help='path to text embeddings pickle file', default='/local-scratch/yuer/projects/text2shape/code/logs/retrieval/v64i128b128/Nov12_05-26-10-2/test/output.p')
    ...
    np.random.seed(1234)
    compute_metrics(args.dataset, embeddings_dict, os.path.dirname(args.embeddings_path), metric=args.metric, concise=render_dir)
