...
def write_nrrd(voxel_tensor, filename, a=0.6):
    ...
    for i in range(voxel_tensor.shape[0]):
        for j in range(voxel_tensor.shape[1]):
            for k in range(voxel_tensor.shape[2]):
                alpha = voxel_tensor[i, j, k, -1]
                if alpha > a:
                    voxel_tensor[i,j,k, 3] = 1.0
                else:
                    voxel_tensor[i,j,k,-1] = 0
...
#     """
#     from tricolo.utils.config import cfg
